...
export function RealtimeCounter() {
  ...
  const handleIncrement = () => {
    const nextValue =
      (optimisticCount !== null ? optimisticCount : count || 0) + 1;
    ...
    <div className="bg-[#2b1810] dark:bg-dark-panel rounded-[2.5rem] p-8 md:p-10 flex flex-col items-center justify-between min-h-[450px] shadow-2xl relative border border-[#4a2b1f] dark:border-dark-border w-full transition-colors duration-200">
      ...
      <div className="flex-1 flex flex-col items-center justify-center w-full mt-12">
        <div className="min-w-32 h-36 px-8 border-[3px] border-dark-primary/20 rounded-[3rem] flex items-center justify-center mb-10 transition-transform active:scale-95 duration-150">
          <span className="text-dark-primary text-7xl font-light tabular-nums">
            {optimisticCount !== null ? optimisticCount : count || 0}
          </span>
        </div>
        <button
...
export default function ActivityTrack() {
  ...
  const [particles, setParticles] = useState<
    Array<{ id: number; bin: number; type: string }>
  >([]);
  ...
  const { stats, activeViewports, viewportRange } = useActivityData(
    ...
    (event) => {
      startTransition(() => {
        const actionBin = Math.min(
          NUM_BINS - 1,
          Math.floor(event.percent / BIN_SIZE),
        );
  ...
  const totalGlobalScrolls = Object.values(scrollBins).reduce(
    (acc: any, val: any) => acc + val,
    0,
  ) as number;
  const totalGlobalClicks = Object.values(clickBins).reduce(
    (acc: any, val: any) => acc + val,
    0,
  ) as number;
  ...
  const rawHeatData = [
    40, 30, 60, 40, 80, 50, 30, 40, 90, 60, 40, 30, 50, 70, 40, 30, 20,
  ];
        ...
        <div className="relative flex items-end w-full h-[60px] pointer-events-auto group">
          ...
          >
            <defs>
              <linearGradient id="areaGradient" x1="0" y1="0" x2="0" y2="1">
                <stop
                  offset="0%"
                  stopColor="rgba(74,43,31,0.15)"
                  className="dark:stop-color-[rgba(244,235,225,0.15)]"
                />
                <stop
                  offset="40%"
                  stopColor="rgba(74,43,31,0.05)"
                  className="dark:stop-color-[rgba(244,235,225,0.05)]"
                />
                <stop
                  offset="100%"
                  stopColor="rgba(43,24,16,0.6)"
                  className="dark:stop-color-[rgba(10,10,10,0.8)]"
                />
              </linearGradient>
            </defs>
          ...
          <div className="absolute inset-0 z-[15] pointer-events-none">
            {particles.map((p, i) => {
              ...
              const gradientColor = isClick
                ? "from-dark-accent/80"
                : "from-white/40 dark:from-dark-primary/30";
              ...
              return (
                ...
                >
                  ...
                  <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 opacity-0 group-hover/bin:opacity-100 transition-opacity pointer-events-none bg-[#2b1810] dark:bg-dark-panel text-dark-primary border border-[#4a2b1f] dark:border-dark-border px-3 py-1.5 rounded-lg text-[10px] font-mono shadow-[0_0_20px_rgba(0,0,0,0.8)] z-[60] whitespace-nowrap flex flex-col items-center">
                    <span className="text-[#d4b8a8] dark:text-dark-secondary pb-1 border-b border-dark-border/50 w-full text-center mb-1">
                      SECTION {binIndex + 1}
                    </span>
                    <span className="font-bold text-xs">
                      {heat}{" "}
                      <span className="font-normal text-[#d4b8a8]/60 dark:text-dark-secondary/60 text-[10px]">
                        Interactions
                      </span>
                    </span>
                  </div>
          ...
          <div className="absolute bottom-full mb-4 right-4 opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none bg-[#2b1810] dark:bg-dark-panel text-dark-primary border border-[#4a2b1f] dark:border-dark-border p-3 rounded-lg text-xs font-mono shadow-[0_0_20px_rgba(0,0,0,0.8)] z-[70] flex items-center gap-4">
            <div className="flex flex-col gap-1 pr-4 border-r border-[#4a2b1f] dark:border-dark-border/50">
              <span className="text-[#d4b8a8] dark:text-dark-secondary font-semibold">
                Realtime Heatmap
              </span>
              <span className="text-[10px] text-[#d4b8a8]/70 dark:text-dark-secondary/70 w-[200px] font-sans leading-snug normal-case tracking-normal">
                Tracks time spent on sections + clicks. Peaks are focus areas.
                Based on {totalGlobalClicks.toLocaleString()} clicks and{" "}
                {(totalGlobalScrolls / 10).toLocaleString()}s of scrolling.
              </span>
            </div>
            <div className="flex flex-col gap-2">
              <div className="flex items-center gap-2">
                <div className="w-2.5 h-2.5 rounded-full bg-red-500 border border-white shadow-[0_0_8px_rgba(239,68,68,0.5)]" />
                <span>You</span>
              </div>
              <div className="flex items-center gap-2">
